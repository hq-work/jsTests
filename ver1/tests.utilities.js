...
function executeUtilitiesTests() {
	module("Utilities", {
		setup: function() {
			this.toAbsolutePath = function(relativePath) {
	...
	test("Build web application URL - Address does not start with '/'", 1, function() {
		...
		// When
		var actualUrl = envianceSdk._buildWebAppUrl(url);

		// Then
		var expectedUrl = this.toAbsolutePath(url);
	...
	test("Build quick link - System ID is null", 1, function() {
		...
		try {
			...
			// Then
			var expectedLink = this.toAbsolutePath("goto/home/ql/" + id);
	...
	test("Build quick link - System ID is not null", 1, function() {
		...
		try {
			...
			// Then
			var expectedLink = this.toAbsolutePath("goto/home/ql/" + id + "?systemId=" + testSystemId);
	...
	test("Build document download link", 1, function() {
		// Given
		var id = createUUID();
		...
		// Then
		var expectedLink = this.toAbsolutePath("Documents/DocumentDownload.aspx?docID=" + id);
		equal(actualLink, expectedLink, "Links are equal.");
	});
