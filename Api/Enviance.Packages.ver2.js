...
/*
 The Enviance.Packages requires jQuery 1.8.3 (/Packages/Libs/jquery.1.8.3.min.js) and JSON2 (/Packages/Libs/json2.js).
 To resolve a conflict with jQuery versions use <noConflict> instruction (http://api.jquery.com/jQuery.noConflict/).
 */

if ((typeof jQuery != "undefined") && !jQuery.ajaxReloaded) {
	// IE10 resolve HttpStauses 401,403 into 0 due to bug of browser. So, we use unused HttpStatuses 419/420 to represent HttpStatuses 401/403
	// Resolving HttpStauses for IE10
	var originalJAjax = jQuery.ajax;
	jQuery.ajax = function () {
		var settings = arguments[0] || {};
		var errorFunc = settings.error;
		settings.error = function (jqXhr, textStatus, errorThrown) {
			if (jqXhr.status == 419) jqXhr.status = 401;
			else if (jqXhr.status == 420) jqXhr.status = 403;
			if (errorFunc) errorFunc(jqXhr, textStatus, errorThrown);
		};
		arguments[0] = settings;
		return originalJAjax.apply(jQuery, arguments);
	};
	jQuery.ajaxReloaded = true;
}
...
(function() {
	...
	/*
	 Enviance Sdk Public Fields declarations
	 */
	envianceSdk.packages.packagesRoot = '/Packages/';
	envianceSdk.packages.stdLibs = '/Packages/Libs/';
	envianceSdk.packages.packageConfig = null;
	envianceSdk.packages.packageId = null;
	envianceSdk.packages.packageWebPath = null;

	/*
	 Enviance Sdk Private declarations
	 */
	function log(msg) {
	...
	function eventSourceHandlerWrapper(handler, args) {
		return function() { return handler.apply(envianceSdk.packages, args); };
	}
	eventSource.prototype = {
		fire: function() {
			...
			for (var i = 0; i < this._handlers.length; i++) {
				...
				this._check(handler);
				defer = defer.pipe(eventSourceHandlerWrapper(handler, arguments));
			}
	...
	//Public Events
	envianceSdk.packages.configStarting = new eventSource();
	envianceSdk.packages.configComplete = new eventSource();
	...
	var _private = {
		...
		_baseAddress: null,
		_sessionTimeout: 180,
		_mainAppSystemId: null,
		_mainAppSessionId: null,
		_mainAppUserId: null,
		_bootStrapAddress: "/Packages/Api/BootStrap.svc",
		_webAppVirtualPath: "",
		_config: null,
		_scriptName: 'enviance.packages.ver2.js',

		_loadAppConfigurationFile: function(oncomplete) {
			return jQuery.ajax({
				url: 'package.cfg',
				dataType: "json",
				async: false,
				success: function(response) {
					envianceSdk.packages.packageConfig = response;
				},
				error: function(xhr, status, msg) {
					log('The package.cfg loading error. ' + msg + '. Status: ' + status);
				},
				complete: function() {
					if (oncomplete) {
						oncomplete();
		...
		_loadAppLocalConfigurationScript: function(oncomplete) {
			return jQuery.ajax({
				url: "package.local.js",
				dataType: "script",
				async: false,
				complete: function() {
					if (oncomplete) {
				...
				error: function(xhr, status, msg) {
					log('The package.local.js loading error. ' + msg + '. Status: ' + status);
				}
			});
		},

		_isHostedAsPackage: function() {
			var hostedUrl = null;
			if (this._webAppVirtualPath == null || this._webAppVirtualPath == '' || this._webAppVirtualPath == '/') {
				var scripts = document.getElementsByTagName('script');
				if (scripts.length > 0) {
					for (var i = 0; i < scripts.length; i++) {
						var script = scripts[i];
						var index = script.src.toLowerCase().lastIndexOf(this._scriptName);
						if (index > 0) {
							hostedUrl = script.src;
							break;
						}
					}
				}
			} else {
				hostedUrl = this._webAppVirtualPath;
			}

			function prepareHostNameString(hostName) {
			...
			function getHostNameFromUrl(url) {
				...
				a.href = url;
				return prepareHostNameString(a.hostname);
			}

			var isHostedAsPackage = false;
			if (hostedUrl != null) {
				var hostname = getHostNameFromUrl(hostedUrl);
				isHostedAsPackage = (hostname == '' || hostname == prepareHostNameString(window.location.hostname.toString()));
			}

			_private._isHostedAsPackage = function() {
				return isHostedAsPackage;
			};

			return isHostedAsPackage;
		},

		_ensureIsHosted: function() {
			if (this._webAppVirtualPath == null) {
				this._webAppVirtualPath = this._resolveWebPath();
			}
			if (navigator.userAgent.indexOf('Mobile') < 0 && window == window.top) {
				document.cookie = "ContentURL=" + escape(location.pathname) + ";path=/;";
				var path = location.host == "localhost:8080"
					? "http://localhost:8080/" //Inside SDK page
					: this._buildWebAppUrl("/MainForm.aspx");
		...
		_setupSessionTimeoutHandling: function() {
			var windowTimeout = (this._sessionTimeout - 1) * 60 - 10; // Same expression as in the EnviancePage.cs
			window.TimerHandle = window.setTimeout(function() {
				if (window.top.onTimer == null) {
					return;
				}
				window.top.onTimer(); //MainForm's onTimer() call (see EnvaincePage.js)

			}, 1000 * windowTimeout);
		},

		checkSessionActive: function(onerror) {
			var self = this;
			return jQuery.ajax({
				type: "GET",
				url: this._buildWebAppUrl(this._bootStrapAddress) + "/2",
				crossdomain: false
			}).then(function(response) {
				if (response.sessionId == null) {
					return "nosession"
				}
				if (response.currentSystemId != self._mainAppSystemId) {
					return "wrongsystem";
				}
				if (self._mainAppUserId && response.userId != self._mainAppUserId) {
					return "wronguser";
				}
				if (response.sessionId != null) {
					self._mainAppSessionId = response.sessionId;
				}
				return "okay";
		 ...
		 logout: function(){
			 //window.navigate("/Authentication/Logout.aspx");

			var returnUrl = envianceSdk.packages['_private']._buildPackageUrl('/');
			 window.location = "/Authentication/Login.aspx?"
				+ "UseCorporateLogin=" + this._mainAppSystemId
				+ "&ReturnUrl=" + encodeURIComponent(returnUrl);
		 },

		//Synchronous method. Resolve some configuration variables. Returns true if success, false if failed.
		_resolveBootStrap: function(onerror) {
			var self = this;
			jQuery.ajax({
				type: "GET",
				url: this._buildWebAppUrl(this._bootStrapAddress) + "/2",
				...
				async: false,
				success: function(response) {
					self._baseAddress = response.apiUrl;
					if (response.sessionId != null) {
						self._mainAppSessionId = response.sessionId;
					}
					if (response.sessionTimeout != null) {
						self._sessionTimeout = response.sessionTimeout;
					}
					if (response.currentSystemId != null) {
						self._mainAppSystemId = response.currentSystemId;
					}
					if (response.userId != null) {
						self._mainAppUserId = response.userId;
					}
				},
				error: function(response, status, message) {
					log('BootStrap resolving error: ' + message);
					if (onerror) {
		...
		_resolveWebPath: function() {
			//MainApp can not be hosted as folder now
			return "/";
		},

		_buildWebAppUrl: function(url) {
			var base = this._webAppVirtualPath;
			if (base == null) {
				throw new Error("_webAppVirtualPath is not set. Can not build URL. Possible, wrong BootStrap state");
			}
			if (url.charAt(0) != "/") {
				url = "/" + url;
			}
		...
		_resolvePackageId: function() {
			//expect that server already set PackageInfo cookie
			var packageInfo = this._readCookie('PackageInfo');
			if (packageInfo) {
				return packageInfo.split(';')[0];
			}
			return null;
		},

		_resolvePackageWebPath: function () {
			//expect that server already set PackageInfo cookie
			var packageInfo = this._readCookie('PackageInfo');
			var result = "/";
			if (packageInfo) {
				var server = packageInfo.split(';')[1];
				//sometimes server have different encoding  than client (like '%20', instead '+')
				result = window.location.pathname.split("/", server.split("/").length - 1).join("/") + "/";
			}
			return result;
		},
		_buildPackageUrl: function (url) {
			var base = envianceSdk.packages.packageWebPath;
			if (base == null) {
				throw new Error("envianceSdk.packages.packageWebPath is not set. Can not build URL. Possible, wrong BootStrap state");
			}
			if (url.charAt(0) != "/") {
				url = "/" + url;
			}
		...
		_buildRequestHeaders: function(denySystemId) {
			var headers = new Object();

			var sessionId = this._mainAppSessionId;
			if (sessionId != null) {
				headers["Authorization"] = "Enviance " + sessionId;
			}

			var systemId = this._mainAppSystemId;
			if (systemId != null && denySystemId !== true) {
				headers["EnvApi-SystemId"] = systemId;
			}

			var packageId = envianceSdk.packages.packageId;
			if (packageId != null) {
				headers["EnvApi-PackageId"] = packageId;
			}
			
			headers["X-Basic-Auth"] = envianceSdk.getEnableBasicAuth();
		...
		_setOptions: function(options) {
			...
			if (options.webAppVirtualPath != null) {
				this._webAppVirtualPath = options.webAppVirtualPath;
			} else {
				this._webAppVirtualPath = this._resolveWebPath();
			}
			...
			if (this._baseAddress == null) {
				if(options.baseAddress && options.sessionId && options.currentSystemId && options.userId){
					//Bootstrap not needed, all already in params
				}else{
					//Synchronous method
					this._resolveBootStrap();
				}
			}

			if (this._baseAddress == null && options.baseAddress != null) {
				this._baseAddress = options.baseAddress;
			} else {
				this._config.baseAddress = this._baseAddress;
			}

			if (typeof (this._baseAddress) == "undefined" || this._baseAddress == null) {
				log('The baseAddress is not defined.');
			}

			if (options.sessionId != null) {
				envianceSdk.packages._private._mainAppSessionId = options.sessionId;
			}
			if (options.sessionTimeout != null) {
				envianceSdk.packages._private._sessionTimeout = options.sessionTimeout;
			}
			if (options.currentSystemId != null) {
				envianceSdk.packages._private._mainAppSystemId = options.currentSystemId;
			}
			if (options.userId != null) {
				envianceSdk.packages._private._mainAppUserId = options.userId;
			}

			if (options.packageId != null) {
				envianceSdk.packages.packageId = options.packageId;
			}

			if (options.packageWebPath != null) {
				envianceSdk.packages.packageWebPath = options.packageWebPath;
			}

			if (options.ensureIsHosted == true) {
				if (!this._ensureIsHosted()) {
					log('The envianceSdk initialization was canceled. The package should be in hosted mode.');
					return false;
				}
			}

			if (options.setupSessionTimeoutHandling == null) {
				options.setupSessionTimeoutHandling = true;
			}
			if (options.setupSessionTimeoutHandling == true) {
				this._setupSessionTimeoutHandling();
			}

			if (options.crossDomainWorkaround != null) {
				envianceSdk.setCrossDomainWorkaround(options.crossDomainWorkaround);
			}
		...
		_configure: function() {
			envianceSdk.configure({
				...
				sessionId: this._mainAppSessionId,
				systemId: this._mainAppSystemId,
				userId: this._mainAppUserId,
				webAppVirtualPath: this._webAppVirtualPath,
				packageId: envianceSdk.packages.packageId,
				refreshPageOnUnauthorized: true
			});

			//May override
			if (this._config != null) {
				envianceSdk.configure(this._config);
			}

			//Check if SDK Runner integration is being running
			if (window.top != null && window != window.top) {
				if (window.top.envianceSdk != null && window.top.envianceSdk.integrateWithSdk) {
					window.top.envianceSdk.integrateWithSdk(envianceSdk);
				}
			}
		}
	};

	var ajax = function(settings, onsuccess, onerror) {
		...
		return jQuery.ajax({
			...
			error: function(jqXhr, textStatus, errorThrown) {
				...
				if (envianceSdk.errorHandler.handle(jqXhr, textStatus, resubmitCallback)) {
					return;
				}
	...
	envianceSdk.packages._private = _private;
	envianceSdk.packages._ajax = ajax;
	var processDatasetResult = function(response) {
		var resultResponse = {
			metadata: response.metadata,
			result: envianceSdk.processDataset(response.result)
		};
		return resultResponse;
	};

	/*
	 Private Package Data Management declarations
	 */
	var dataStorageValidator = {
		checkKeys: function(keys, checkIsEmpty) {
			if (!isArray(keys)) {
				throw new Error("The argument <keys> is not an array.");
			}
			if (checkIsEmpty && keys.length == 0) {
				throw new Error("The argument <keys> is empty.");
			}
			var duplicates = {};
			for (var i = 0; i < keys.length; i++) {
				var key = keys[i];
				this._checkKey(key);
				if (typeof (duplicates[key.toLowerCase()]) == 'undefined') {
					duplicates[key.toLowerCase()] = true;
				} else {
					throw new Error("The array contains a duplicate key <" + key + ">.");
				}
			}
		},
		checkDictionary: function(dictionary) {
			if (typeof (dictionary) != 'object' || isArray(dictionary)) {
				throw new Error("The 'dictionary' argument is not an object.");
			}
			var duplicates = {};
			for (var key in dictionary) {
				...
				if (typeof (duplicates[key.toLowerCase()]) == 'undefined') {
					...
					throw new Error("The dictionary contains a duplicate key: <" + key + ">.");
				}
				this._checkValue(key, dictionary[key]);
			}
		},
	...
	var dataStorage = function(provider, validator) {
		this._provider = provider;
		this._validator = validator;
	};

	dataStorage.prototype = {
		get: function(keys, onsuccess, onerror) {
			this._validator.checkKeys(keys, false);
			return this._provider.get(keys, onsuccess, onerror);
		},
		getAsync: function (keys, onsuccess, onerror) {
			this._validator.checkKeys(keys, false);
			return this._provider.getAsync(keys, onsuccess, onerror);
		},
		set: function(dictionary, onsuccess, onerror) {
			this._validator.checkDictionary(dictionary);
			return this._provider.set(dictionary, onsuccess, onerror);
		},
		del: function(keys, onsuccess, onerror) {
			this._validator.checkKeys(keys, true);
			return this._provider.del(keys, onsuccess, onerror);
		}
	};


	var defaultDataStorageProvider = function() {
	};
	defaultDataStorageProvider.prototype = {
		get: function(keys, onsuccess, onerror) {
			var dictionary = null;
			ajax({
				type: "POST",
				url: this._getUrl(),
				async: false,
				data: "getDataParam=" + encodeURIComponent(JSON.stringify(keys)),
				contentType: "application/x-www-form-urlencoded; charset=UTF-8"
			},
				function(response) {
					if (onsuccess) {
						try {
							dictionary = JSON.parse(response.result);
						} catch (e) {
		...
		set: function(dictionary, onsuccess, onerror) {
			return ajax({
				type: "POST",
				url: this._getUrl(),
				data: "saveDataParam=" + encodeURIComponent(JSON.stringify(dictionary)),
				contentType: "application/x-www-form-urlencoded; charset=UTF-8"
			}, onsuccess, onerror);
		},
		del: function(keys, onsuccess, onerror) {
			return ajax({
				type: "POST",
				url: this._getUrl(),
				data: "deleteDataParam=" + encodeURIComponent(JSON.stringify(keys)),
				contentType: "application/x-www-form-urlencoded; charset=UTF-8"
			}, onsuccess, onerror);
		},
		_getUrl: function() {
			throw new Error("The _getUrl method is not implemented.");
		}
	};

	var defaultUserDataStorageProvider = new defaultDataStorageProvider();
	defaultUserDataStorageProvider._getUrl = function() {
		return _private._buildPackageUrl("/_UsrData/" + _private._mainAppUserId);
	};

	var defaultAppDataStorageProvider = new defaultDataStorageProvider();
	defaultAppDataStorageProvider._getUrl = function() {
		return _private._buildPackageUrl("/_PkgData");
	};

	/*
	Private Package Shims declarations
	*/
	var localDataStorageProvider = function() {
	};
	localDataStorageProvider.prototype = {
		get: function(keys, onsuccess, onerror) {
			var dictionary = {};
			try {
				dictionary = keys.length == 0 ? this._getAllData() : this._getDataByKeys(keys);
			} catch (e) {
				if (onerror) {
					onerror(e);
				}
		...
		set: function(dictionary, onsuccess, onerror) {
			for (var key in dictionary) {
				try {
					this._localStorage().setItem(this._getLocalStorageKey(key), dictionary[key]);
				} catch (e) {
					if (onerror) {
						onerror(e);
					}
		...
		del: function(keys, onsuccess, onerror) {
			for (var i = 0; i < keys.length; i++) {
				var key = keys[i];
				try {
					this._localStorage().removeItem(this._getLocalStorageKey(key));
				} catch (e) {
					if (onerror) {
						onerror(e);
					}
		...
		_getAllData: function() {
			var dictionary = {};
			var lStorage = this._localStorage();
			for (var i = 0, l = lStorage.length; i < l; i++) {
				var lStorageKey = lStorage.key(i);
				var prefix = this._getLocalStorageKey('');
				if (lStorageKey.indexOf(prefix) == 0) {
					var key = lStorageKey.replace(prefix, '');
					dictionary[key] = lStorage.getItem(lStorageKey);
				}
			}
			return dictionary;
		},
		_getDataByKeys: function(keys) {
			var dictionary = {};
			for (var i = 0; i < keys.length; i++) {
				var key = keys[i];
				var value = this._localStorage().getItem(this._getLocalStorageKey(key));
				if (value != null) {
		...
		_localStorage: (function() {
			if (typeof window.localStorage == "object") {
				return function() { return window.localStorage; };
		...
		_getLocalStorageKey: function() {
			throw new Error("The _getLocalStorageKey method is not implemented.");
		}
	};

	var localAppDataStorageProvider = new localDataStorageProvider();
	localAppDataStorageProvider._getLocalStorageKey = function(key) {
		if (typeof (envianceSdk.packages.packageId) != 'string' || envianceSdk.packages.packageId.length != 36) {
			throw new Error("The packageId is not set.");
		}
		return envianceSdk.packages.packageId + '_appdata_' + key;
	};

	var localUserDataStorageProvider = new localDataStorageProvider();
	localUserDataStorageProvider._getLocalStorageKey = function(key) {
		if (typeof (envianceSdk.packages.packageId) != 'string' || envianceSdk.packages.packageId.length != 36) {
			throw new Error("The packageId is not set.");
		}
		var userId = _private._mainAppUserId == null ? '00000000-0000-0000-0000-000000000000' : _private._mainAppUserId;
		return envianceSdk.packages.packageId + '_' + userId + '_userdata_' + key;
	};

	/*
	 Enviance Sdk Public Methods declarations
	 */
	envianceSdk.packages.init = function (options, oncomplete) {
		var resultDeferred = jQuery.Deferred();

		if (_private._initialized) {
			log('The envianceSdk is already initialized.');
		...
		var enablePackageConfig = (typeof options.enablePackageConfig == "undefined") ? true : options.enablePackageConfig;
		var enablePackageLocalScript = (typeof options.enablePackageLocalScript == "undefined") ? false : options.enablePackageLocalScript;

		if (enablePackageLocalScript && !_private._isHostedAsPackage()) {
			//Synchronous method
			_private._loadAppLocalConfigurationScript();
		}
		if (enablePackageConfig === true) {
			//Synchronous method
			_private._loadAppConfigurationFile();
		}

		envianceSdk.packages.configStarting.fire(options)
			.fail(function () { log('There is an error in envianceSdk.packages.configStarting.fire'); })
			.always(function () {
				if (!_private._setOptions(options)) {
					resultDeferred.reject(options);
					return;
				}
				_private._configure();

				var isConfigCompleteFailed = false;
				envianceSdk.packages.configComplete.fire(options)
					.fail(function () {
						log('There is an error in envianceSdk.packages.configComplete.fire');
						isConfigCompleteFailed = true;
					})
					.always(function () {
						if (oncomplete) {
							oncomplete();
						}
						_private._initialized = true;

						if (isConfigCompleteFailed) {
							resultDeferred.reject(options);
						} else {
							resultDeferred.resolve(options);
						}
					});
			});
		return resultDeferred.promise();
	};
	envianceSdk.packages.setOptions = function(options) {
		envianceSdk.packages._private._setOptions(options);
		envianceSdk.packages._private._configure();
	};
	envianceSdk.packages.getOptions = function () {
		var sdkprivate = envianceSdk.packages._private;
		return {
			webAppVirtualPath: sdkprivate._webAppVirtualPath,
			baseAddress: sdkprivate._config.baseAddress,
			packageId: envianceSdk.packages.packageId,
			sessionId: sdkprivate._mainAppSessionId,
			systemId: sdkprivate._mainAppSystemId,
			currentSystemId: sdkprivate._mainAppSystemId,
			userId: sdkprivate._mainAppUserId,
			packageWebPath: sdkprivate._config.packageWebPath,
			refreshPageOnUnauthorized: sdkprivate._config._refreshPageOnUnauthorized,
			sessionTimeout: sdkprivate._sessionTimeout,
			setupSessionTimeoutHandling: false,
			crossDomainWorkaround: envianceSdk._private._crossDomainWorkaround
		};
	};
	/*
	 Utilities declarations
	 */
	envianceRegisterNamespace("envianceSdk.packages.utilities");
	envianceRegisterNamespace("envianceSdk.packages.utilities.uri");

	/*Returns references to the package contents. For instance, when run locally,
	 the reference would just be /path/to/file or path/to/file or ../path/to/file.
	 But run on the server, it would be /contenthandler?/path/to/file etc.*/
	envianceSdk.packages.utilities.uri.toPackageFile = function(path) {
		if (path == null) {
			throw new Error("Argument Exception. Argument 'path' can not be null");
		}
		if (path.length > 0 && path.substr(0, 1) == "/") {
			path = path.substring(1, path.length);
		}
		var basepath = location.protocol + "//" + location.host + location.pathname;
		var pos = basepath.indexOf("CustomApp/");
		if (pos > 0) {
			return basepath.substring(0, pos + 10 + 36) + "/" + path;
		} else {
			throw new Error("State Exception. Can not calculate basepath");
		}
	};

	envianceSdk.packages.utilities.uri.toQuickLink = function(id) {
		return _private._buildWebAppUrl("/Mvc/Pages/StartMvcView.aspx?ChartID=QuickLink_Execute&StateID=ES_Execute&instanceId=" + id);
	};

	envianceSdk.packages.utilities.uri.toUnexpectedErrorPage = function() {
		return _private._buildWebAppUrl("Error/UnhandledError.htm");
	};

	/*
	 Package Data Management declarations
	 */
	envianceSdk.packages.userData = new dataStorage(defaultUserDataStorageProvider, dataStorageValidator);
	envianceSdk.packages.appData = new dataStorage(defaultAppDataStorageProvider, dataStorageValidator);
	envianceSdk.packages.packageId = _private._resolvePackageId();
	envianceSdk.packages.packageWebPath = _private._resolvePackageWebPath();

	/*
	 Package Shims
	 */
	envianceSdk.packages.localAppDataShim = new dataStorage(localAppDataStorageProvider, dataStorageValidator);
	envianceSdk.packages.localUserDataShim = new dataStorage(localUserDataStorageProvider, dataStorageValidator);

	envianceSdk.packages.userLocalShim = function(rolesJsonFile) {
		this._userName = null;
		this._rolesJsonFile = rolesJsonFile;
		this._userRolesMap = null;
	};
	envianceSdk.packages.userLocalShim.prototype = {
		authenticate: function(username, password, onsuccess, onerror) {
			var self = this;
			return envianceSdk.authentication.authenticate(
				username,
				password,
				function(response) {
		...
		hasAccessTo: function() {
			return true;
		},

		isInRole: function(roleName) {
			if (this._userName == null) {
				throw new Error("The user name is not defined. Please use userLocalShim.authentication to set user name.");
			}
			if (typeof (roleName) != 'string') {
				throw new Error("The roleName is not a string.");
			}
			var map = this._getUserRolesMap();
			if (map != null) {
				for (var user in map) {
					if (user.toLowerCase() == this._userName.toLowerCase()) {
						var roles = map[user];
		...
		_getUserRolesMap: function() {
			if (this._userRolesMap == null) {
				var self = this;
				this._loadRoles(
					function(map) { self._userRolesMap = map; },
					function() { self._userRolesMap = null; }
				);
			}
			return this._userRolesMap == null ? {} : this._userRolesMap;
		},

		_loadRoles: function(onsuccess, onerror) {
			return jQuery.ajax({
				url: this._rolesJsonFile,
				dataType: "json",
				async: false,
				success: function(response) {
					if (typeof (response) != 'object') {
						throw new Error("The Role File Shim data is not an object.");
					}
					for (var user in response) {
						var roles = response[user];
						if (isArray(roles)) {
							for (var i = 0; i < roles.length; i++) {
								if (typeof (roles[i]) != 'string') {
									throw new Error("The item of roles array is not a string.");
								}
							}
						} else {
							throw new Error("The roles is not an array.");
						}
					}
					if (onsuccess) {
						onsuccess(response);
	 ...
	 */
	envianceRegisterNamespace("envianceSdk.packages.user");

	// This is a no-op function which can be overridden if needed in the package.local.js file
	// in order to handle authentication outside of the Enviance App runtime.
	envianceSdk.packages.user.authenticate = function(username, password, onsuccess, onerror) {
	};


	envianceSdk.packages.user.isInRoleAsync = function(roleName) {
		var def = jQuery.Deferred();
		ajax({
				type: "GET",
				url: _private._buildPackageUrl("/_PkgRoles/" + roleName),
				cache: false
			},
			function(response) {
				def.resolve(response.metadata.statusCode == 204);
			},
			function() {
				def.resolve(false);
			});
	...
	envianceSdk.packages.user.availableRolesAsync = function() {
		var def = jQuery.Deferred();

		ajax({
				type: "GET",
				url: _private._buildPackageUrl("/_PkgRoles"),
				cache: false
			},
			function(response) {
				def.resolve(envianceSdk.JSON.parse(response.result));
			},
			function() {
				def.resolve(null);
			});
	...
	envianceSdk.packages.user.hasAccessToAsync = function(itemPath) {
		envianceSdk.packages.user._checkPath(itemPath);
		...
		ajax({
				type: "HEAD",
				url: _private._buildPackageUrl(itemPath),
				cache: false
			},
			function(response) {
				def.resolve(response.metadata.statusCode == 200);
			},
			function() {
				def.resolve(false);
			});
	...
	envianceSdk.packages.user.isInRole = function(roleName) {
		var result = true;
		ajax({
			type: "GET",
			url: _private._buildPackageUrl("/_PkgRoles/" + roleName),
			...
			function(response) {
				result = response.metadata.statusCode == 204;
			},
			function() {
				result = false;
			});
	...
	envianceSdk.packages.user.availableRoles = function() {
		var result = null;
	...
	envianceSdk.packages.user.hasAccessTo = function(itemPath) {
		envianceSdk.packages.user._checkPath(itemPath);
		if (itemPath.charAt(0) != "/") {
			itemPath = "/" + itemPath;
		}
	...
	envianceSdk.packages.user._checkPath = function(path) {
		if (typeof (path) != "string" || path == "") {
			throw new Error("The 'path' argument is not a string or is empty.");
		}
		if (path.length > 255) {
			throw new Error("The 'path' argument cannot be greater than 255 characters.");
		}
	};

	/*
	Package stored EQL query
	*/
	envianceRegisterNamespace("envianceSdk.packages.eql");

	envianceSdk.packages.eql.ExecuteParametersInfo = function(path, format, page, pageSize) {
		if (path) {
			this.path = path;
		}
		if (format) {
	...
	envianceSdk.packages.eql.executeEqlItem = function(executeParameters, queryParameters, onsuccess, onerror) {
		if (typeof executeParameters !== "object" || executeParameters == null) {
			throw new Error("The \"executeParameters\" required and should be object.");
		}
		if (typeof executeParameters.path !== "string") {
			throw new Error("The \"path\" property in \"executeParameters\" object is required.");
		}
		if (executeParameters.path.charAt(0) != "/") {
			executeParameters.path = "/" + executeParameters.path;
		}

		return ajax({
			type: "POST",
			url: _private._buildPackageUrl(executeParameters.path),
			data: {
				eqlQueryParam: JSON.stringify({
					parameters: queryParameters,
					page: executeParameters.page,
					pageSize: executeParameters.pageSize,
					format: executeParameters.format
				})
			},
			contentType: "application/x-www-form-urlencoded; charset=UTF-8"
		}, function(response) {
			if (onsuccess != null) {
				if (executeParameters.format == null || executeParameters.format.toString().toUpperCase() === "JSON") {
					onsuccess(processDatasetResult(response));
				} else {
					onsuccess(response);
				}
			}
	...
	envianceSdk.packages.eql.executeEqlItemBinary = function (executeParameters, queryParameters, onsuccess, onerror) {
		if (typeof executeParameters !== "object" || executeParameters == null) {
			throw new Error("The \"executeParameters\" required and should be object.");
		}
		if (typeof executeParameters.path !== "string") {
		...
		return envianceSdk.formDataSubmit({
			type: "POST",
			url: _private._buildPackageUrl(executeParameters.path),
			data: {
				eqlQueryParam: JSON.stringify({
					...
					pageSize: executeParameters.pageSize,
					format: executeParameters.format,
					filename: executeParameters.filename,
					cachecontrol: executeParameters.cachecontrol
				})
			},
			contentType: "application/x-www-form-urlencoded; charset=UTF-8"
		}, function (response) {
...
envianceSdk = (function(envianceSdk) {
	...
	*/
	...
	_private._formDataSubmit = function (settings, onsuccess, onerror) {
		...
		if (container == null) {
			...
			form.append(jQuery("<input />", { name: "X-Base-Auth", value: envianceSdk.getEnableBasicAuth(), type: "hidden" }));
	...
	*/
	...
	_private._enableBasicAuth = true;
	...
	*/
	...
	envianceSdk.configure = function(options) {
		...
		if (options.enableBasicAuth != null) {
			_private._enableBasicAuth = options.enableBasicAuth;
		}
	};
	...
	envianceSdk.getEnableBasicAuth = function () {
		return _private._enableBasicAuth;
	};
...
envianceSdk = (function(envianceSdk) {
	...
	// Non-Numeric Data
	envianceSdk.data.enterNonNumericData = function (nonNumericDataPoints, collector, onsuccess, onerror) {
		//Check for old version
		if (arguments.length < 4 && typeof (collector) == "function") {
			onerror = onsuccess;
			onsuccess = collector;
		...
		return _private._ajax({
			type: "POST",
			url: _private._buildUrl('ver2/DataService.svc/nonnumericdata/enterdatacommands'),
			data: envianceSdk.JSON.stringify({ "nonNumericDataPoints": nonNumericDataPoints, "collector": collector }),
			success: function (response, textStatus, xhr) {
				if (onsuccess) {
					_private._runCommandPolling(_private._processResult(response, xhr), onsuccess, onerror);
				}
	...
	envianceSdk.data.enterNonNumericDataAsync = function (nonNumericDataPoints, collector, onsuccess, onerror) {
		//Check for old version
		if (arguments.length < 4 && typeof (collector) == "function") {
			onerror = onsuccess;
			onsuccess = collector;
	...
	envianceSdk.data.deleteNonNumericData = function (requirementIdOrPath, nonNumericDataPoints, onsuccess, onerror) {
		return _private._ajax({
			type: "POST",
			url: _private._buildUrl('ver2/DataService.svc/nonnumericdata/deletedatacommands'),
			data: envianceSdk.JSON.stringify({ "requirementIdOrPath": requirementIdOrPath, "nonNumericDataPoints": nonNumericDataPoints })
		}, onsuccess, onerror);
	};
	...
	envianceSdk.workflows.getWorkflowStepBulk = function(workflowIdOrUniqueIdTostepIdOrNameMap, onsuccess, onerror) {
		return _private._ajax({
				type: "POST",
				url: _private._buildUrl('ver2/WorkflowService.svc/workflows/steps'),
				data: envianceSdk.JSON.stringify({ "workflowIdOrUniqueIdTostepIdOrNameMap": workflowIdOrUniqueIdTostepIdOrNameMap })
			}, onsuccess, onerror);
	};
	envianceSdk.workflows.getWorkflowCurrentStepBulk = function(workflowIdsOrUniqueIds, onsuccess, onerror) {
		return _private._ajax({
				type: "POST",
				url: _private._buildUrl('ver2/WorkflowService.svc/workflows/steps/currentsteps'),
				data: envianceSdk.JSON.stringify({ "workflowIdsOrUniqueIds": workflowIdsOrUniqueIds })
			}, onsuccess, onerror);
	};
	...
	envianceSdk.workflows.reopenWorkflow = function (workflowIdOrUniqueId, onsuccess, onerror) {
		var stepInfo = new envianceSdk.workflows.WorkflowStepInfo();
		stepInfo.setTransition("Reopen Workflow");
		return envianceSdk.workflows.updateWorkflowCurrentStep(workflowIdOrUniqueId, stepInfo, onsuccess, onerror);
	};
	...
	envianceSdk.groups.setGroupMembersAsync = function(groupIdOrName, members, onsuccess, onerror) {
		return _private._ajax({
			type: "PUT",
			url: _private._buildUrl('ver2/SecurityService.svc/groups/' + encodeURIComponent(groupIdOrName) + '/members/Async'),
			data: envianceSdk.JSON.stringify({ "members": members })
		}, onsuccess, onerror);
	};
	...
	envianceSdk.groups.addGroupMembersAsync = function (groupIdOrName, members, onsuccess, onerror) {
		return _private._ajax({
			type: "PATCH",
			url: _private._buildUrl('ver2/SecurityService.svc/groups/' + encodeURIComponent(groupIdOrName) + '/members/Async'),
			data: envianceSdk.JSON.stringify({ "members": members })
	...
	envianceSdk.materialData.deleteMaterialData = function (locationIdOrPathOrUniqueTag, mdlGroupIds, waitForCompletion, onsuccess, onerror) {
		return _private._ajax({
			type: "POST",
			url: _private._buildUrl('ver2/MaterialDataService.svc/materialdata/deletedatacommands/' + _private.uriPathEncode(locationIdOrPathOrUniqueTag) + ''),			
	...
	envianceSdk.materialData.deleteMaterialDataAsync = function (locationIdOrPathOrUniqueTag, mdlGroupIds, waitForCompletion, onsuccess, onerror) {
		return _private._ajax({
			type: "POST",
			url: _private._buildUrl('ver2/MaterialDataService.svc/materialdata/deletedatacommands/' + _private.uriPathEncode(locationIdOrPathOrUniqueTag) + ''),			
			data: envianceSdk.JSON.stringify({ "mdlGroupIds": mdlGroupIds})
	...
	envianceSdk.materialData.getMaterialData = function (activityIdOrNameOrUniqueTag, completeBegin, completeEnd, locationIdOrPathOrUniqueTag, onsuccess, onerror) {
		...
		return _private._ajax({
			type: "GET",
			url: _private._buildUrl('ver2/MaterialDataService.svc/materialdata/' + encodeURIComponent(activityIdOrNameOrUniqueTag) +
				'/begin/' + encodeURIComponent(completeBegin) + '/end/' + encodeURIComponent(completeEnd) + '/' + _private.uriPathEncode(locationIdOrPathOrUniqueTag))
		}, onsuccess, onerror);
	};
	...
	envianceSdk.materialData.uploadMaterialData = function (fileinput, objectIdOrPathOrUniqueTag, onsuccess, onerror) {
		var validationResults = envianceSdk.common.validateUploadFiles([fileinput]);
		if (validationResults.length > 0) {
			var errorMessage = '';
			for (var i = 0; i < validationResults.length; i++) {
		...
		return _private._formDataSubmit({
			...
			data: envianceSdk.JSON._preProcess({ "objectIdOrPathOrUniqueTag": objectIdOrPathOrUniqueTag })
		}, onsuccess, onerror);
	};
...
envianceSdk = (function(envianceSdk) {
	...
	envianceSdk.data.CompliancePeriod = function (beginDate, endDate) {
		this.beginDate = beginDate;
		this.endDate = endDate;
	};
	...
	envianceSdk.tasks.TaskOccurrenceInfo = function (objectIdOrPath, dismissed, percentComplete, hoursToComplete, costToComplete, comment, statusChangeDate, eventInstanceId) {
		if (objectIdOrPath) {
	...
	envianceSdk.data.NonNumericDataPointInfo = function (requirementIdOrPath, completeDate, collector, comments, compliancePeriod, comply, taskId) {
		if (requirementIdOrPath) {
			this.requirementIdOrPath = requirementIdOrPath;
		}
		if (completeDate) {
		...
		if (compliancePeriod) {
			this.compliancePeriod = new envianceSdk.data.CompliancePeriod(compliancePeriod.beginDate, compliancePeriod.endDate);
		}
		if (comply) {
			this.comply = comply;
		}
	...
	envianceSdk.materialData.MdlGroupInfo = function (parentIdOrPathOrUniqueTag, activityIdOrNameOrUniqueTag, complete, collector, dataLines) {
		if (parentIdOrPathOrUniqueTag) {
			this.parentIdOrPathOrUniqueTag = parentIdOrPathOrUniqueTag;
		}
		if (activityIdOrNameOrUniqueTag) {
			this.activityIdOrNameOrUniqueTag = activityIdOrNameOrUniqueTag;
		}
		if (complete) {
		...
		this.addDataLine = function (dtId, materialIdOrNameOrUniqueTag, mdlGroupId, startTime, endTime, data) {
			this.dataLines = this.dataLines || [];
			this.dataLines.push(new envianceSdk.materialData.MaterialDataLinesInfo(dtId, materialIdOrNameOrUniqueTag, mdlGroupId, startTime, endTime, data));
			return this;
		};
	};

	envianceSdk.materialData.MaterialDataLinesInfo = function (dtId, materialIdOrNameOrUniqueTag, mdlGroupId, startTime, endTime, data) {
		if (dtId) {
			this.dtId = dtId;
		}
		if (materialIdOrNameOrUniqueTag) {
			this.materialIdOrNameOrUniqueTag = materialIdOrNameOrUniqueTag;
		}
		if (mdlGroupId) {
		...
		this.addData = function (dataLineId, propertyIdOrNameOrUniqueTag, value) {
			this.data = this.data || [];
			this.data.push(new envianceSdk.materialData.MaterialDataInfo(dataLineId, propertyIdOrNameOrUniqueTag, value));
		};
	};

	envianceSdk.materialData.MaterialDataInfo = function (dataLineId, propertyIdOrNameOrUniqueTag, value) {
		if (dataLineId) {
			this.id = dataLineId;
		}
		if (propertyIdOrNameOrUniqueTag) {
			this.propertyIdOrNameOrUniqueTag = propertyIdOrNameOrUniqueTag;
		}
		if (value) {
			this.value = value;
		}
	...
	envianceSdk.materialData.MdlGroupUploadInfo = function (name, description, content, fileName, mimeType, objectIdOrPathOrUniqueTag) {
		if (name) {
			this.name = name;
		}
		if (description) {
		...
		if (objectIdOrPathOrUniqueTag) {
			this.objectIdOrPathOrUniqueTag = objectIdOrPathOrUniqueTag;
		}
	};
	...
	envianceSdk.requirements.NonNumericRequirementInfo = function (requirementType, name, parentIdOrPath, activeDate, inactiveDate,
		responsibleUserIdOrName, description, requirementTemplateIdOrName, specificCitation, periodFrequency, periodTime, complianceStatement) {
		if (requirementType) {
		...
		if (complianceStatement) {
			this.complianceStatement = complianceStatement;
		}
	};
	...
	// all Requirements
	//Non-NumericRequirementInfo
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.addDocument = envianceSdk.common.addDocument;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.setResponsibleUser = envianceSdk.requirements.SetResponsibleUser;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.setFieldTemplate = envianceSdk.common.setFieldTemplate;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.addScalarFieldValue = envianceSdk.common.addScalarFieldValue;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.addScalarFieldHistValue = envianceSdk.common.addScalarFieldHistValue;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.addDateFieldValue = envianceSdk.common.addDateFieldValue;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.addDateFieldHistValue = envianceSdk.common.addDateFieldHistValue;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.addTimeFieldValue = envianceSdk.common.addTimeFieldValue;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.addTimeFieldHistValue = envianceSdk.common.addTimeFieldHistValue;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.addUrlFieldValue = envianceSdk.common.addUrlFieldValue;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.addLinkedFieldValues = envianceSdk.common.addLinkedFieldValues;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.addMultiFieldValues = envianceSdk.common.addMultiFieldValues;
	envianceSdk.requirements.NonNumericRequirementInfo.prototype.addTag = envianceSdk.common.addTag;
